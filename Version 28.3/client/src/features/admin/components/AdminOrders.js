                            ...
                            <table className="w-full table-auto">
                                <thead>
                                    <tr className="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                                        <th className="py-3 px-6 text-left cursor-pointer flex items-center" onClick={(e) => handleSort({ sort: 'id', order: sort?._order === 'asc' ? 'desc' : 'asc' })}>Order ID {
                                            sort._sort === 'id' && (
                                                sort._order === 'desc' ?
                                        ...
                                        <th className="py-3 px-6 text-center cursor-pointer flex items-center" onClick={(e) => handleSort({ sort: 'totalAmount', order: sort?._order === 'asc' ? 'desc' : 'asc' })}>Total Amount {
                                        ...
                                        <tr key={index} className="border-b border-gray-200 hover:bg-gray-100">
                                            <td className="py-3 px-6 text-left whitespace-nowrap">
                                                <div className="flex items-center">
                                                    <span className="font-medium text-lg">#{order.id.slice(-4)}</span>
                                                </div>
                                            </td>
                                            <td className="py-3 px-6 text-left">
                                                {order.items.map((item, index) => (
                                                    <div key={index} className="flex items-center py-1">
                                                        ...
                                                        <span className='font-medium'>{item.product.title} - ${item.product.price - Math.round(item.product.price * item.product.discountPercentage / 100)} - ({item.quantity})</span>
                                                    </div>
                                                ))}
                                            </td>
