...
import { fetchLoggedInUserOrdersAsync, selectUserInfo, selectUserOrders, selectUserStatus } from '../userSlice';
                        ...
                        <div className="flex justify-between gap-x-6 py-1">
                            <div className="flex min-w-0 gap-x-4">
                                <div className="min-w-0 flex-auto">
                                    <p className="text-md font-semibold leading-6 text-gray-900">{order.selectedAddress[0].name}</p>
                                    <p className="mt-1 truncate text-xs leading-5 text-gray-500">Phone: {order.selectedAddress[0].phone}</p>
                                    <p className="mt-1 truncate text-xs leading-5 text-gray-500">Email: {order.selectedAddress[0].email}</p>
                                </div>
                            </div>
                            <div className="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
                                <p className="text-sm leading-6 text-gray-700">{order.selectedAddress[0].houseNumber} - {order.selectedAddress[0].locality}</p>
                                <p className="text-sm leading-6 text-gray-500">{order.selectedAddress[0].city}</p>
                                <p className="text-sm leading-6 text-gray-500">{order.selectedAddress[0].state} - {order.selectedAddress[0].pinCode}</p>
                            </div>
                        </div>
