...
import { deleteItemFromCartAsync, selectCartItemsStatus, selectItems, updateCartAsync } from './cartSlice';
...
export default function Cart() {
  ...
  const totalAmount = items.reduce((amount, item) => {
    let itemPrice = item.price - Math.round((item.price * item.discountPercentage / 100));
  ...
  const handleQuantity = (e, item) => {
    ...
    toast.info(`Quantity of ${item.title} changed to ${e.target.value}`, {
  ...
  return (
    <>
      ...
      <div className='mx-auto mt-24 max-w-7xl px-4 sm:px-6 lg:px-8 bg-white rounded-lg'>
